...
| Kubernetes / Helm / GitOps / image scan / SBOM | **Not present** in-repo (no charts, no scan workflow). CI (`.github/workflows/api-gateway.yml`) runs typecheck + `test:unit` only. **Planning:** [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md); **founder decision worksheet:** [staging-platform-decision-record](./staging-platform-decision-record.md) (not deploy evidence; approvals pending) |
...
| Kubernetes/GitOps | Blocked | No charts/workflows for deploy; planning contract [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md); **decision worksheet** [staging-platform-decision-record](./staging-platform-decision-record.md) (approvals pending — not In progress) | First GitOps path only after worksheet Section 6 approvals + implementation plan | Cluster, registry, scan/provenance, secret injection, billing/account | Platform (external); BLACKNOIX founder (manifests when unblocked) |
...
| Kubernetes/platform hardening | Cluster compromise; unscanned images | Hardened deploy path; provenance; gated cutover | Platform scan/provenance; chart cutover only after gates | Local Compose only; no Helm/GitOps/scan config in-repo; ADR-0013 packaging baseline (historical); [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md); [staging-platform-decision-record](./staging-platform-decision-record.md) | TBD (planning/worksheet only — not deploy evidence) | Blocked / external | Platform (external); BLACKNOIX founder (app manifests when unblocked) | Cluster, scan, provenance; founder Section 6 approvals incomplete | TBD | Blocked |
