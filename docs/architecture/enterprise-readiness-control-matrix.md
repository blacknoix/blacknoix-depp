...
| Kubernetes / Helm / GitOps / image scan / SBOM | **No live deploy/push path.** Non-deployable `infra/gitops/` ([aws-eks-gitops-skeleton](../runbooks/aws-eks-gitops-skeleton.md)). **Non-operational** image supply-chain workflow skeleton: `.github/workflows/staging-image-supply-chain.yml` + [aws-github-oidc-image-supply-chain](../runbooks/aws-github-oidc-image-supply-chain.md) (placeholder-gated; no ECR/OIDC provisioned) |
...
| Kubernetes/GitOps | Blocked | Non-deployable GitOps skeleton + **non-operational** OIDC/ECR supply-chain workflow skeleton (guards fail on REPLACE_*; no cloud provision) | Founder AWS/GitHub OIDC+ECR setup; then first gated workflow_dispatch; then GitOps digest PR | Account, ECR, IAM OIDC trust, protected environment | Platform (external); BLACKNOIX founder |
