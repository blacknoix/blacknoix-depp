...
| Kubernetes / Helm / GitOps / image scan / SBOM | **No live deploy/push path.** Non-deployable `infra/gitops/` ([aws-eks-gitops-skeleton](../runbooks/aws-eks-gitops-skeleton.md)). **Non-operational** image supply-chain workflow: `.github/workflows/staging-image-supply-chain.yml` binds cloud job to GitHub Environment `staging` + [aws-github-oidc-image-supply-chain](../runbooks/aws-github-oidc-image-supply-chain.md) (placeholder-gated; environment approval ≠ ECR/deploy evidence) |
...
| Kubernetes/GitOps | Blocked | Non-deployable GitOps skeleton + **non-operational** OIDC/ECR supply-chain workflow (`supply-chain` → `environment: staging`; guard outside env; no cloud provision) | Founder AWS/GitHub OIDC+ECR setup; then first gated workflow_dispatch; then GitOps digest PR | Account, ECR, IAM OIDC trust, protected environment | Platform (external); BLACKNOIX founder |
...
| Kubernetes/platform hardening | Cluster compromise; unscanned images | Hardened deploy path; provenance; gated cutover | Platform scan/provenance; chart cutover only after gates | Local Compose; ADR-0013 historical; decision record; **non-deployable** `infra/gitops/` skeleton | Skeleton cite only — not deploy evidence | Blocked / external | Platform (external); BLACKNOIX founder | Account access; CI scan; ESO; Argo; real digest | TBD | Blocked |
