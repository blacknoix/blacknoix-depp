...
| Kubernetes / Helm / GitOps / image scan / SBOM | **Not present** in-repo (no charts, no scan workflow). CI (`.github/workflows/api-gateway.yml`) runs typecheck + `test:unit` only. **Planning prerequisite:** [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md) (not deploy evidence) |
...
| Kubernetes/GitOps | Blocked | No charts/workflows for deploy; **planning contract** [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md) (owner decisions required before manifests) | First GitOps path only after Section 4 approvals + implementation plan | Cluster, registry, scan/provenance, secret injection | Platform (external); BLACKNOIX founder (manifests when unblocked) |
...
4. **Planning (this gate):** [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md) — owner decisions required before any Helm/GitOps/deploy slice.
