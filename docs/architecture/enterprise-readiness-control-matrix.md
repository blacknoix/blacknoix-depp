...
| Kubernetes / Helm / GitOps / image scan / SBOM | **No live deploy path.** CI typecheck/`test:unit` only. Founder choices recorded in [staging-platform-decision-record](./staging-platform-decision-record.md). **Non-deployable repo skeleton:** `infra/gitops/` + [aws-eks-gitops-skeleton](../runbooks/aws-eks-gitops-skeleton.md) + `scripts/validate-gitops-skeleton.cjs` (placeholder image; no AWS provision) |
...
| Kubernetes/GitOps | Blocked | Founder choices recorded; **non-deployable** Kustomize skeleton in `infra/gitops/` ([aws-eks-gitops-skeleton](../runbooks/aws-eks-gitops-skeleton.md)) — not provisioned, not In progress | AWS account access + explicit authorize for CI scan pipeline / ESO / Argo apply | Account, ECR, scan, secrets, RDS, Auth0 | Platform (external); BLACKNOIX founder |
