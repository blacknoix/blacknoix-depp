...
describe("resolveExplicitRolesMode", () => {
  it("defaults unset/empty to compat and rejects invalid values", () => {
    ...
    assert.throws(() => resolveExplicitRolesMode("strict"), /AUTH_EXPLICIT_ROLES_MODE/);
  });
});
...
describe("principal role helpers (compat)", () => {
  ...
  it("allows auditor-only to read audit and tenant self but not manage agents, query telemetry, or findings", () => {
  ...
  it("lets operator+auditor manage agents, read audit, query telemetry, findings, and tenant self", () => {
  ...
  it("denies agents for audit/manage but allows telemetry query and findings list", () => {
    const agent = {
      tenantId,
