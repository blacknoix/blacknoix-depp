...
| Kubernetes / Helm / GitOps / image scan / SBOM | **Not present** in-repo (no charts, no scan workflow). CI typecheck/`test:unit` only. Planning: [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md). **Founder pilot baseline recorded** (not implemented): [staging-platform-decision-record](./staging-platform-decision-record.md) — AWS EKS Auto Mode, Argo CD, ECR, GHA+OIDC, Trivy/Syft/Cosign, Secrets Manager, RDS, Auth0 |
...
| Kubernetes/GitOps | Blocked | No charts/workflows for deploy; planning [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md); **founder pilot choices recorded** in [staging-platform-decision-record](./staging-platform-decision-record.md) (AWS/EKS Auto Mode / Argo CD / ECR / Secrets Manager / RDS / Auth0 — **not provisioned**; not In progress) | Non-secret GitOps skeleton only after AWS account access + explicit authorize; then scan pipeline / secrets contract / deploy | Account access, registry, scan/provenance, secret injection, billing | Platform (external); BLACKNOIX founder (manifests when unblocked) |
...
| Kubernetes/platform hardening | Cluster compromise; unscanned images | Hardened deploy path; provenance; gated cutover | Platform scan/provenance; chart cutover only after gates | Local Compose only; ADR-0013 historical; [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md); [staging-platform-decision-record](./staging-platform-decision-record.md) (choices recorded, **not** deployed) | TBD (worksheet only — not deploy evidence) | Blocked / external | Platform (external); BLACKNOIX founder | AWS account access + explicit authorize for skeleton; then provision/scan/deploy slices | TBD | Blocked |
...
4. **Planning:** [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md) + founder [staging-platform-decision-record](./staging-platform-decision-record.md) — complete approval checklist before any Helm/GitOps/deploy slice.
5. **Defer implementation until approved:** Windows agent, Dockerfile/verifier changes, ADR-0012/0013 edits, staging IdP packs, and any cluster apply.
...
|---|---|
| Kubernetes / GitOps environment | No in-repo deploy path yet; see [staging-vertical-slice-prerequisites](../runbooks/staging-vertical-slice-prerequisites.md) |
